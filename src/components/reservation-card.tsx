...
export function ReservationCard({ slot, onBack }: ReservationCardProps) {
  ...
  if (!slot || typeof slot.capacitySimple === 'undefined' || typeof slot.capacityDouble === 'undefined') {
    return (
        <Card className="max-w-2xl mx-auto">
            ...
            <CardContent><p>Chargement des détails du créneau...</p></CardContent>
        </Card>
    );
  }
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    const result = await addBooking(slot.id, values);
